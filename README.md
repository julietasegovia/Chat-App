...
### Whispers
To make a private message to a user (which we called whispers), the client should click the name of the desired receptor on the top bar where the online users are displayed. To manage this I used the whisperTo() function. whisperTo() does not support for a user to send whispers to themselves. Also added a condition to the renderMessage function that styles the whispers differently from regular messages and mentions, adding the 'whisper' badge. There's also a 'private message sent' event that notifies that the user's whisper was sent correctly, wich has different styling from a regular message.